...
AbstractModel3D *AbstractModel3D::fromJSON(const json &jsonModel, const SpotLight *spotLight) {
...
//    transformations = rotate(transformations,0,vec3(jsonModel["rotate"]["x"], jsonModel["rotate"]["y"], jsonModel["rotate"]["z"]));
	...
	if (type == "light_texture") {
		...
		return LightTexModel3D::create("assets/models/" + mesh, "assets/textures/" + texture, material, transformations, spotLight);
	}
	if (type == "normal") {
		string mesh = jsonModel["args"]["objPath"];

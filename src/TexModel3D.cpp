...
TexModel3D::TexModel3D(const string &meshPath, const string &texturePath, const mat4 &modelTransform) : AbstractModel3D(
        meshPath, "shaders/tex3D.fs.glsl", modelTransform) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageTexture->getWidth(), imageTexture->getHeight(), 0, GL_RGBA, GL_FLOAT,
